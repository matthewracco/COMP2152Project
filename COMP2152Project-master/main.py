...
import random
from feature import random_chance_event
  ...
  @        ..@#% @     @&*#@(         % 
   &   (  @    (   / /   *    @  .   /  
    ...
    if m_health_points <= 0:
        ...
        # Random chance event after defeating monster
        print("    ------------------------------------------------------------------")
        print("    |    After defeating the monster, something unexpected happens...")
        health_change, message, new_items = random_chance_event()
        print(f"    |    {message}")

        # Apply changes from the event
        health_points = max(0, health_points + health_change)
        belt.extend(new_items)

        if new_items:
            print(f"    |    Added to your belt: {', '.join(new_items)}")
            belt.sort()
            print("    |    Your updated belt:", belt)
